...
LOCK_FILE="${LOCK_FILE:-$ROOT/runs/gpt_bias_fusion_factory_surface_v4/factory.lock}"
mkdir -p "$(dirname "$LOCK_FILE")"

exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "another surface-v4 model factory process holds $LOCK_FILE" >&2
  exit 73
fi
...
while true; do
  attempt=$((attempt + 1))
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] surface-v4 model factory attempt ${attempt}"
  "$TRAIN_BATCH" --config "$CONFIG"
  status=$?
  if [[ $status -eq 0 ]]; then
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] surface-v4 model factory completed"
    exit 0
  fi
